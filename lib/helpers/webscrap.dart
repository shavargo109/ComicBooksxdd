...
class WebScrap {
  Future<http.Response> _getReponse(String url, [headerr]) async {
  ...
  Map<String, dynamic> _decode(
      String function, int a, int c, List<String> data) {
  ...
  Future<List<Book>> fetchLatestBooks(bool isLatest) async {
    try {
      final response =
          await _getReponse('https://tw.manhuagui.com/update/', HEADER);
  ...
  Future<List<Episode>> fetchEpisode(int code) async {
    ...
    try {
      final response = await _getReponse(
          'https://tw.manhuagui.com/comic/$codeString/', HEADER);
  ...
  Future<Map<String, dynamic>> getEpsiodeImage(String link) async {
    ...
    final res = await _getReponse("https://tw.manhuagui.com$link", HEADER);
    ...
    if (match != null) {
      ...
      if (decompressedData != null) {
        final episodeData =
            _decode(function, a, c, decompressedData.split('|'));
        return episodeData;
      }
    }
    return {};
  ...
  Stream<Uint8List> getImagedataStream(
  ...
  Future<int> searchResultPage(String keyword) async {
    ...
    try {
      final response = await _getReponse(
          'https://tw.manhuagui.com/s/${keyword}_p1.html', HEADER);
      if (response.statusCode == 200) {
  ...
  Future<List<Book>> searchResultContent(String keyword, int page) async {
    ...
    try {
      ...
      final res = await _getReponse(
          'https://tw.manhuagui.com/s/${keyword}_p$p.html', HEADER);
  ...
  Future<List<String>> search759() async {
    try {
      final head = {
        'User-Agent':
            'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36', // 模擬 Chrome 瀏覽器
        'Accept':
      ...
      final res = await _getReponse(
          'https://www.759store.com/cecweb/promotion.php?lang=cht', head);
      // final res = await _getReponse('https://www.aeonstores.com.hk/wednesday_freshday');
      if (res.statusCode == 200) {
        List<String> imageLinks = [];
        final document = parser.parse(utf8.decode(res.bodyBytes));
        final results = document.querySelectorAll('.nice_scroll')[0];
        final promoteImages = results.querySelectorAll("li");
        for (final image in promoteImages) {
          var imageLink = image.querySelector('a')?.attributes['href'];
          imageLink = 'https://www.759store.com/${imageLink!}';
  ...
  Future<void> searchAEON() async {
    try {
      final res =
          await _getReponse('https://www.aeonstores.com.hk/wednesday_freshday');
      ;

      if (res.statusCode == 200) {
        final document = parser.parse(utf8.decode(res.bodyBytes));
        final result = document.querySelector('.pdf_box');
        final pdf = result?.querySelector('a')?.attributes['href'];
        final pdfLink = 'https://www.aeonstores.com.hk$pdf';
        print(pdfLink);
        final pdff = await PdfDocument.openUri(Uri.parse(pdfLink));
      }
    } catch (e) {
      print('Error: $e');
    }
